...
import com.cardpay.pccredit.report.service.ReviewRiskControlService;
 ...
 */
@Controller
@RequestMapping("/report/reviewriskcontrol/*")
@JRadModule("report.reviewriskcontrol")
public class ReviewRiskControlController extends BaseController {
	
	@Autowired
	private ReviewRiskControlService reviewRiskControlService;
	...
	public AbstractModelAndView browse(@ModelAttribute StatisticalFilter filter, HttpServletRequest request) {
		...
		List<ReviewRiskControl> list = reviewRiskControlService.getReviewRiskControl(filter);

		JRadModelAndView mv = new JRadModelAndView("/report/reviewriskcontrol/reviewriskcontrol_browse", request);
