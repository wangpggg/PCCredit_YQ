...
//获取xml文件
function Init(v1,v2,v3) {
    //打开xlmdocm文档
    xmlDoc = loadXmlFile( contextPath +'/area/Area.xml');
        ...
        if (TopnodeList.length > 0) {
            ...
            if (province.length> 0) {
                ...
                for (var i = 0; i < TopnodeList[0].childNodes.length; i++) {
                    ...
                    //默认为第一个城市的第一个县列表
                    city = province[childnodeindex].getElementsByTagName("country");;
                    var option = document.createElement("option");
                    option.value = city[i].getAttribute("name");
                    option.text = city[i].getAttribute("name");
                    if (v3 == option.value) {
                        option.selected = true;
                    }
                    this.document.getElementById("Select3").add(option);
                }
...
function selectCity() {
    ...
    if (window.ActiveXObject) {
        ...
        countycodes = citynodes[0].getElementsByTagName("country")
    }
...
function selectcounty() {
    ...
    var city = document.getElementById("Select3");  
    var countys = TopnodeList[dropElement1.selectedIndex].getElementsByTagName("city")[dropElement2.selectedIndex].getElementsByTagName("country")
    RemoveDropDownList(city);
