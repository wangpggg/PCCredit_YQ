...
function countTotal2(){
	...
	//流动资产
	  ...
	  //资产总计
	   ...
	   zczj = $("input[name ='18']").val(countsumtotal1.toFixed(2)); 
	   
	   //短期负债
	  countsum2 = countTotal1("3","5","7","9");
	  $("input[name ='1']").val(countsum2); 
	 //负债总计
	    countsumtotal2 = countTotal1("1","11","13","15");
	 
	   fzzj  =  $("input[name ='16']").val(countsumtotal2); 
	   //所有者权益
	   syzqy = Number(countsumtotal1) - Number(countsumtotal2);
	   
	   $("input[name ='17']").val(syzqy.toFixed(2));
		 
	   //负债和所有者权益总计
	   $("input[name ='19']").val(Number(countsumtotal2)+Number(syzqy));
	   
	   //资产负债率
	   if(countsumtotal1 != 0){
	        fzl = Number(countsumtotal2)/ Number(countsumtotal1) *100;
	        $("input[name ='21']").val(fzl.toFixed(2));
	   }
       ...
       if(dqfz2 != 0){
 		   numb3 = Number(ldzc) / Number(dqfz2) *100 ;
 	   }
       $("input[name ='22']").val(numb3.toFixed(2));
       return;
};
