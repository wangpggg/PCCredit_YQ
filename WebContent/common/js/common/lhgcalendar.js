 ...
 * 目前的用法有两种：
 * 1.带时分秒
 * $("#input").calendar({ format:'yyyy-MM-dd HH:mm:ss' });
 * 2.不带时分秒
 * $("#input").calendar({ btnBar:false });
 * 3.其他用法
 * $("#icon").calendar({ id:'#input' });
 * $("#start").calendar({ maxDate:'#end',btnBar:false });
 * $("#end").calendar({ minDate:'#start',btnBar:false });
 */
...
lhgcalendar.setting =
{
	...
	targetFormat: null,
	disWeek: null,
	onSetDate: function(){$("#"+this.inpE.id).keyup();},
	real: null,
	disDate: null,
	enDate: false,
	zIndex: 1978,
